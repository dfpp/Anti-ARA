...
def start_instrumenting(package, release_thread=False, onstop=None, timeout=None):
    ...
    def run():
        out = request_pipe(cmd)
        logging.info(out)
        if "Process crashed" in out:
            #logging.info("Process crashedProcess crashedProcess crashedProcess crashedProcess crashedProcess crashedProcess crashed")
            #stop_instrumenting(package,timeout)
            crashed.set()
            logging.info("The application exited unexpectedly, but fortunately we kept the most recent code coverage ^_^")
        if "Lock file exists" in out:
            crashed.set()
    ...
    print("Press Ctrl+C to finish or wait 5 minute")
    signal.signal(signal.SIGINT, stop)
    
    time.sleep(2)
    
    if not crashed.is_set():
        t1 = threading.Thread(target=set_crashed)
        t1.start()
